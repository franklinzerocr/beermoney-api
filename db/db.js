...
async function getLastMonthTradingPlans(dbConnection) {
  try {
    ...
    let result = await dbConnection.query('SELECT tp.ID ID ,tp.Asset Asset,tp.Init Init,tp.End End, f.NetProfit FROM trading_plan tp, floor f WHERE tp.OutputAmount>0 AND tp.End IS NOT NULL AND tp.StopCondition IS NOT NULL AND tp.StopCondition NOT LIKE "%ERROR%" AND tp.StopCondition NOT LIKE "%NOT%" AND ' + timeConditionEnd + ' AND ' + floorCondition + ' ORDER BY tp.End DESC');
    return result;
  } catch (e) {
    console.log(e);
